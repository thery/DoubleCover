...
(*  Search.v                                                                  *)
(*                                                                            *)
(*  The lower-bound search: an iterative-deepening depth-first search with an *)
(*  admissible pruning heuristic (IDA star), and the only thing one has to    *)
(*  prove about it -- when it answers "no" there really is no word.           *)
(*                                                                            *)
(*  WHY IT IS SHORT.  The heuristic h is a parameter, subject to two purely   *)
(*  LOCAL conditions:                                                         *)
(*                                                                            *)
(*      h 1 = 0        and       h g <= (h (g * m)).+1  for every move m      *)
(*                                                                            *)
(*  Those two give h g <= d for every g in the d-ball (h_ball), which is all  *)
(*  the search needs to cut a branch.  Nothing else about h is assumed: the   *)
(*  pruning table it is read from need not be exact, need not be complete,    *)
(*  and need not even be injectively indexed.  A bad table makes the search   *)
(*  slower, never unsound.  That is what keeps the trusted part of a          *)
(*  10^11-node computation down to the fifty lines below.                     *)
(*                                                                            *)
(*  ball_split2 is the other half: it breaks one search of depth d.+2 into    *)
(*  the independent searches of depth d that sit under each pair of first     *)
(*  moves.  Those have no shared state, so they are separate lemmas, in       *)
(*  separate files, checked in parallel.                                      *)
(* =========================================================================  *)

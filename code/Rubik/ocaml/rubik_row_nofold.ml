(* One row of the upper bound, the computation Rokicki's hcoset does.
   ...
   usage: rubik_row_nofold <cap> <maxdepth> "<moves>" [<maxsearch>]   one row
          rubik_row_nofold <cap> <maxdepth> build                    the table only
          rubik_row_nofold check                                     the checks
          rubik_row_nofold hball <n>                                 the prepass alone
          rubik_row_nofold fball <n>                                 the same, folded
          rubik_row_nofold pball <n>                       the same, persistent arrays
...
let lrow n =
  ...
  if not (Sys.file_exists path) then begin
    prerr_endline "phase1_cap9.tbl is not there: ./rubik_row_nofold 9 20 build";
    exit 1 end;
...
let () =
  ...
  if Array.length Sys.argv > 2 && Sys.argv.(1) = "dumptab" then begin
    ...
    Buffer.add_string out
      "(* GENERATED by ocaml/rubik_row_nofold.ml dumptab -- do not edit.        \
       \     *)\n\

...
(*                                                                            *)
(* OBLIGATION C IS TWO THINGS, and the second is small.                       *)
(*                                                                            *)
(*   THE MOVE ACTS ON THE DATUM.  Not on the number: on what the number is a  *)
(*     ranking of.  For the edges that datum is where the four slice cubies   *)
(*     sit and how they lie, four places out of twelve with a flip each; a    *)
(*     turn sends it to another such datum by a rule that does not look at    *)
(*     the position at all.  This is the mathematics of C.                    *)
(*   THE TABLE IS THE RANKING OF THAT RULE, checked datum by datum.  There    *)
(*     are 12 * 11 * 10 * 9 ordered placements and a flip each, 190080, which *)
(*     is n_e exactly, so the check is a sweep of 190080 x 12 and it needs no *)
(*     table of its own.                                                      *)
(*                                                                            *)
(* WHAT THIS ROUTE DOES NOT NEED, and the earlier note in this file asked for *)
(* all three: no unranking (cube_of_e stays in the prototype), no round trip, *)
(* and no injectivity of the ranking.  Sweeping the DATA rather than the      *)
(* NUMBERS is what removes them: the position's number is the rank of its     *)
(* datum, so a rule checked at every datum answers for every position, and    *)
(* nothing has to be recovered from a number.                                 *)
(*                                                                            *)
(* The same twice more, smaller: the four U corners among eight places, 70,   *)
(* and the seven free twists, 2187.                                           *)
(*                                                                            *)
(* MEASURED, and it is what says the route is real: a turn carries the twelve *)
(* edge facelet pairs to edge facelet pairs, primary to primary or secondary  *)
(* consistently, for all twelve turns -- so the place a cubie comes from and  *)
(* whether it is turned over are read off the move alone.  The one thing the  *)
(* position must satisfy is cubti, that the two facelets of an edge stay      *)
(* together, and Coordfsi has that closed under the turns.                    *)
(*                                                                            *)

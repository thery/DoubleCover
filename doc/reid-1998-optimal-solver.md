# Michael Reid, "new optimal solver", Cube Lovers, 31 July 1998

Kept verbatim.  It is the second Reid post this development depends on; the
first is `reid-1998-fourspot.md`, which states the 26q lower bound and the
three propositions that reduce it to six searches.  This one says what his
solver was, and it is the only account we have of the table that made those
searches affordable: a quotient of 29 099 347 200 cosets, exact to distance
14 in quarter turns.

The distance columns below are what our own table has to reproduce, coset for
coset, if our coordinate is the same one.
...
Both columns sum to 190080 * 70 * 2187 = 29 099 347 200 exactly, and
12094 * 70 * 2187 = 1 851 470 460 nibbles is 925 735 230 bytes, his 883 Mb.
The mail is internally consistent.

The mean quarter-turn distance is 11.55 and the mean face-turn distance
10.06.  Fed into the usual estimate for iterative deepening -- sequences of
length n times the fraction of cosets within the remaining depth, with the
canonical quarter-turn branching 8.899 -- his six searches come to about
6.9e11 nodes, which at a million nodes a second is his measured 153 hours.
That estimate is ours, and his node rate is not in the mail.

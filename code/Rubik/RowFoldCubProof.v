(* =========================================================================  *)
(*  RowFoldCubProof.v -- what the folded run's answer buys.                   *)
(* =========================================================================  *)

(* rowfulliO = true -> the row of the superflip is within twenty.  Nothing is *)
(* computed here: RowFoldCubBool settles the boolean and RowFoldCubDone puts  *)
(* the two together.                                                          *)
(*                                                                            *)
(* rowmapiD is RowFoldNSim's yfwmfinoD, whose certificate gives the row, and  *)
(* rowmapiO is rowmapiD: each search is shown equal to RowFoldN's at the same *)
(* tables, by induction on the depth left, with RowOpt's equations for the    *)
(* faster reads and RowFoldLvlG's for the level.                              *)
(*                                                                            *)
(* NO GOAL BELOW COMPARES TWO SEARCHES BY CONVERSION.  One step of each       *)
(* fixpoint is unfolded by cbv with the fixpoint named, and every difference  *)
(* is closed by an equation.                                                  *)
...
Require Import RowFold RowFoldTab RowFoldSrch RowRunConst.
Require Import RowFoldSrchI RowFoldSrchIC RowLeafFast.
Require Import RowCoord RowCoordLeaf RowFoldN RowFoldNSim.
Require Import RowOpt RowFoldLvlG RowFoldCubDef.
...
(* ---- the names RowRunConst copies ---------------------------------------- *)

(* each pair is equal by unfolding a short function                           *)
...
(* ---- the run is RowFoldNSim's, so it is sound ---------------------------- *)
...
(* ---- rowmapiD's searches, at its tables ---------------------------------- *)

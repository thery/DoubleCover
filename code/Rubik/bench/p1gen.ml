(* The phase 1 table -- twist x flip x slice, 2 217 093 120 states -- built in
   ...
   usage: p1gen [cap] [mode]   cap defaults to 9, as rubik_par's does
     ...
     emit       emit P1Data.v and P1Data_real.v: the table itself *)
...
let () =
  ...
  if Array.length Sys.argv > 2 && Sys.argv.(2) = "emit" then begin
    ...
    header oc "DUMMY -- empty stand in for P1Data_real.v.  Swap it in only \
               when the certificate is being checked.  (srank and twmove are \
               NOT here: they are small, so they live in P1Small.v, which is \
               checked in and always required.)";
    ...
    header oc "The phase 1 table, chunk 0 of 71.";

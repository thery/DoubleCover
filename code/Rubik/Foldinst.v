...
(*  Foldinst.v                                                                *)
(*                                                                            *)
(*  Fold.v takes the folding data as variables and its properties as          *)
(*  hypotheses.  This file supplies both: the emitted tables, and a boolean   *)
(*  check for each hypothesis.  What comes out is the orbit check in place    *)
(*  of the rank sweep -- Farp1.p1checkStepr from Fold.foldcheckStep.          *)
(*                                                                            *)
(*  Every check here is a computation over emitted numbers, so every proof    *)
(*  is the same three steps: state the loop, vm_compute it, and read the      *)
(*  instance back out with rank_of_check, twist_of_check, sym_of_check or     *)
(*  move_of_check.                                                            *)
(* =========================================================================  *)
...
(* the rank loop: 2 ^ 20 values with the ones past nfsi guarded out.  This is
   Foldtab.frepL's proof, said once. *)
...
(* THE RANK TABLE IS A VARIABLE.  Requiring the emitted one to STATE the
   ...
   with it -- Foldcert.v instantiates it once. *)
Section Obl.
...
(* and the four folding tables, Foldtab.v's frepi, fsymi, twsymi and repsi.
...
(* THE CHECKS ARE HYPOTHESES HERE.  Running them needs the emitted tables and
   ...
   proved once against a section hypothesis and Foldcert.v pays the bill.
...
(* the symmetry a rank folds by is one of the sixteen.  Foldtab has the same

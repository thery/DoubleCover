...
(* THE SAME RUN OVER THE GUARDED LEVEL.  Writing to a persistent array leaves *)
(* an undo record on the old version, and that record lives as long as        *)
(* anything still points at the old version.  A level writes 613 million      *)
(* words, and the map as it stood BEFORE the level is still held -- it is the *)
(* argument -- so all 613 million records stay alive until the level ends.    *)
(* That is the memory.                                                        *)
(*                                                                            *)
(* On a sparse map nearly every one of those writes puts a nought back where  *)
(* a nought already was.  The guarded level tests first and does not write,   *)
(* so no record is made.  Filliatre's own persistent array tests; Rocq's      *)
(* PArray does not.                                                           *)
(*                                                                            *)
(* RowFoldChk.v is left exactly as it was, so the two can be run side by      *)
(* side.  Both must print 3072 then 2560.                                     *)

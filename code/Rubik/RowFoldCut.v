(* =========================================================================  *)
(*  RowFoldCut.v -- the row with hcoset's last two cuts.                      *)
(* =========================================================================  *)

(* WHAT IS NEW.  RowFoldSrch.fsrchk adds the two cuts the prototype has and   *)
...
Time Eval native_compute in frunc 10 0 0%uint63 (mkempty tt) (mkempty tt) [::].

(* to thirteen: the cuts are still off, so this must be the same list and the
   same time as the run without them -- 2560, 72832, 1192960, 14731320 *)
Time Eval native_compute in frunc 13 0 0%uint63 (mkempty tt) (mkempty tt) [::].

(* and to fourteen, where they come on.  The prototype's count is
   148 423 860. *)
Time Eval native_compute in frunc 14 0 0%uint63 (mkempty tt) (mkempty tt) [::].

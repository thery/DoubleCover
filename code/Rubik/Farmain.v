...
(* the second move outermost, so that the eighteen pieces are exactly the
   eighteen conjuncts *)
Lemma searchd :
  all (fun j => all (fun i => ~~ searchz5 droot (prefixi i j) (init5 (prefixi i j)) nfcube)
                    (iota 0 nroot))
      (iota 0 nmoves).
...
have hs : searchz5 droot (prefixi i j) (init5 (prefixi i j)) nfcube = false.
...
exact: (far_of_searchz5 (d := droot) (prefixi_ok iL' jL) (prefixi_cub iL' jL) hs).
Qed.

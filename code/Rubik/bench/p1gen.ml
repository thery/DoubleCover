...
let emit_table () =
  ...
  Printf.fprintf oc
    "(* GENERATED by bench/p1gen.ml -- do not edit.                     *)\n\
     ...
     (* The chunks are primitive array literals.  See doc/p1table.md.   *)\n\n\
     From Stdlib Require Import Uint63.\n\
     From Stdlib Require Import PArray.\n\
     Require Import"
    nchunk cwords;
  ...
  if Array.length Sys.argv > 2
    ...
    if Sys.argv.(2) = "emitfold" then begin
      ...
      chk "twsym" (fun i ->
        ...
        twsym 200_000;
      (* the rank under each symmetry, flat, and the rank of each orbit's
         representative.  Fold.v needs both: the first to say which
         symmetries carry a rank to its representative, the second to name
         the representative itself. *)
      let nract = nfs * nsym in
      let racta = Array.init nract (fun i -> symfs.(i mod nsym).(i / nsym)) in
      let reps = Array.init norb (fun i -> canon.(i)) in
      chk "ract" (fun i ->
        (pack3 racta nract (i / sw) lsr ((i mod sw) * sbits)) land mask20)
        racta 200_000;
      chk "reps" (fun i ->
        (pack3 reps norb (i / sw) lsr ((i mod sw) * sbits)) land mask20)
        reps 200_000;
      let fbad = ref 0 in
      ...
      emit_arr oc "reps_data" ((norb + sw - 1) / sw) (pack3 reps norb);
      close_out oc;
      wrote "../P1Fold.v";

      (* the rank under each symmetry, chunked like the folded table *)
      let rwords = (nract + sw - 1) / sw in
      let rnchunk = (rwords + cwords - 1) / cwords in
      for c = 0 to rnchunk - 1 do
        let lo = c * cwords in
        let hi = min (lo + cwords) rwords in
        let fn = Printf.sprintf "../P1R_%02d.v" c in
        let oc = open_out fn in
        header_arr oc (Printf.sprintf
          "The rank under each of the sixteen symmetries, chunk %d of %d: \
           words %d .. %d.  Three ranks to a word at twenty bits."
          c rnchunk lo (hi - 1));
        emit_arr oc (Printf.sprintf "ract_chunk_%02d" c) (hi - lo)
          (fun j -> pack3 racta nract (lo + j));
        close_out oc;
        wrote fn
      done;
      let oc = open_out "../P1RTable.v" in
      Printf.fprintf oc
        "(* GENERATED by p1gen -- do not edit. *)\n\n\
         From Stdlib Require Import Uint63.\n\
         From Stdlib Require Import -(notations) PArray.\n";
      for c = 0 to rnchunk - 1 do
        Printf.fprintf oc "Require Import P1R_%02d.\n" c
      done;
      Printf.fprintf oc
        "\nDefinition ractab : PArray.array (PArray.array int) :=\n\
        \  let a := PArray.make %d (PArray.make 1 0%%uint63) in\n" rnchunk;
      for c = 0 to rnchunk - 1 do
        Printf.fprintf oc
          "  let a := PArray.set a %d%%uint63 ract_chunk_%02d in\n" c c
      done;
      Printf.fprintf oc "  a.\n";
      close_out oc;
      wrote "../P1RTable.v";
      ...
      Printf.fprintf oc
        "(* GENERATED by bench/p1gen.ml -- do not edit.              *)\n\
         ...
         (* %d words, %d entries of twenty eight bits.         *)\n\n\
         From Stdlib Require Import Uint63.\n\
         From Stdlib Require Import PArray.\n\

(* =========================================================================  *)
(*  ScrCut.v -- stt_cut: a cut throws no maneuver away.                      *)
(* =========================================================================  *)
...
Require Import Table Tabi Rubik333 Sym Moves Coordfs Coordfsi Phase1
        ...
        HSound HEdge HRunS HCorner HSweep HSweepC HGlue.
...
(* ---- the axis view, copied from HPok.v ---------------------------------- *)
...
(* HSweepC.sweeps_agree delivers them, one lookup on the real table, the      *)
(* range of the coordinates, and the four facts about the axis view that      *)
(* HPok.v owes.                                                               *)
...
Hypothesis Hview_ok : forall a i, tabi_ok flast a -> (i < nax)%N ->
  tabi_ok flast (view i a).

Hypothesis Hview_step : forall a i m, tabi_ok flast a -> (i < nax)%N ->
  (m < nq)%N ->
  ti2t flast (view i (comp_tabi flast a (mvq m)))
    = ti2t flast (comp_tabi flast (view i a) (mvq (cmv i m))).

Hypothesis Hhtriple_eq : forall a b, tabi_ok flast a -> tabi_ok flast b ->
  ti2t flast a = ti2t flast b -> htriple a = htriple b.

Hypothesis Hpok_step : forall a m, pok a -> (m < nq)%N ->
  pok (comp_tabi flast a (mvq m)).

Hypothesis Hstt_sol : forall a, pok a -> eq_tabi flast a idi -> hsolved (stt a).
...
have pk' : pok (comp_tabi flast a (mvq m)) by apply: Hpok_step.
rewrite aw_cons.
by apply: ih.
Qed.
...
have [k0 k1 k2] : [/\ heq (htriple (view 0 b)), heq (htriple (view 1 b)) &
                      heq (htriple (view 2 b))] := hsolvedP (Hstt_sol pk hs).

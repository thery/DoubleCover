...
bound. Every position of the cube is solved in 20 moves or fewer.

Take a subgroup $H$ of $G$. For a position $x$, the _coset_ $x H$ is the
set of the positions $x h$ with $h$ in $H$. Every position lies in
exactly one coset. We take one position $x$ in each coset. It is the
_representative_ of the coset. It is then enough to show that every
position of $x H$ is solved in 20 moves or fewer.

For one coset, we use a _marking_. Each position of $x H$ gets one bit.
...
or fewer. Doing this for every representative gives the upper bound.

This is the main idea: the marking is local to one coset. The work on
one coset does not depend on the work on another. So the problem splits
into many independent problems. They can be done in parallel. A coset
has as many positions as $H$, with one bit for each. So if $H$ is not
too large, the memory needed is small.

The choice of $H$ is crucial. The next subsection presents the one we
take.

== Cosets

We have already seen the subgroup $H$ we take. It is the set of the
positions whose phase 1 summary is the solved one. A coset of $H$ is
one value of the summary. There are as many cosets as summaries.

$H$ has 19 508 428 800 positions. So a coset is 19 508 428 800 bits,
...
We use 2 facts from @lowerbound.

- *Checking membership is quick.* A position $p$ is in $x H$ exactly
  ...
  reach is in $H$, we set the bit of $h$.
- *The distance to the solved summary is known.* The phase 1 table gives
  it. For a position, this is the number of moves needed to bring it into
  $H$. We use it to cut the enumeration, as in the search of
  @lowerbound. We
  build the words of length $d$ one move at a time. Say $k$ moves have
  been applied, and the table gives $t$. If $k + t > d$, no word that
  continues from there can end in $H$ after $d$ moves. We drop this
  branch. The table is checked to be never too large (@verify). So a cut
  never drops a word that ends in $H$.

What is new is that $H$ is generated by 10 of the 18 moves: `U`, `U2`,
`U'`, `D`, `D2`, `D'`, `R2`, `L2`, `F2` and `B2`. This is a result of
Kociemba. These are the moves that do not change the summary. So $H$
splits the 18 moves in 2 sets. The 10 moves of $H$ do not
change the summary, so they do not change the distance to $H$. The other
8 moves, `R`, `R'`, `L`, `L'`, `F`, `F'`, `B` and `B'`, change the
summary. They can change the distance.
...
not empty has a least member. The file is 80 lines and assumes nothing.

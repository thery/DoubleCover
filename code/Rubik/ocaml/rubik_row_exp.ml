(* rubik_row_exp.ml -- rubik_row_fold.ml with switches to take the search    *)
(* apart, 2026-09-25.  EXP_BITRANK: the rank by a bit mask.  EXP_LAST:     *)
(* hcoset's last level, children marked straight from the mask.  EXP_NOMARK, *)
(* EXP_NOWRITE, EXP_NOSTEP, EXP_NOCNT, EXP_LATE, EXP_MLEV=0|1|2: ablations.  *)
(* The rank is typed int array, which the committed file is not.          *)
(* One row of the upper bound, the computation Rokicki's hcoset does.
   ...
   usage: rubik_row_fold <cap> <maxdepth> "<moves>" [<maxsearch>]  one row
          rubik_row_fold <cap> <maxdepth> build             the plain table
          rubik_row_fold <cap> <maxdepth> fold              the folded table
          rubik_row_fold <cap> foldchk [n]                  folded against plain
          rubik_row_fold check                              the checks
          rubik_row_fold hball <n>                          the prepass alone
          rubik_row_fold fball <n>                          the same, folded
          rubik_row_fold pball <n>                  the same, persistent arrays
   ...
   THIS IS rubik_row_nofold WITH THE HEURISTIC FOLDED.  Beside the distance
   the search wants to know WHICH MOVES GO CLOSER -- that is what hcoset's
   phase1prune carries, and it is what turns eighteen tries and eighteen
   table reads a node into three or four of each.  Written out state by
   state that table is a word each of 2 217 093 120 states, 17.7 GB, which
   is what rubik_row_nofold reads and why a node of ours costs what it does.

   The sixteen renamings that keep the U/D axis send a state to a state at
   the same distance, so one state of each orbit is enough.  They act on the
   flip x slice rank and on the twist separately, and 1 013 760 ranks fall
   into 64 430 orbits, so the table is 64 430 x 2187 entries, phase1f<K>.tbl.

   AN ENTRY IS FOUR BYTES, and that is Rokicki's own packing.  Writing the
   moves out one bit each takes forty one bits; but U2 is U1 twice, so two
   moves of one face cannot differ by more than one step, which leaves
   fifteen of the twenty seven ways a face can go.  Four bits a face, six
   faces, four bits of distance: twenty eight, and the table is 563 MB.

   A state is read in two steps: its rank names the orbit and a renaming
   that carries the rank to the orbit's kept one, and the twist is played
   through that same renaming.  What comes back is the KEPT state's word, so
   the moves it names have to be renamed back before they are played.

   Build it with `fold', which reads phase1_cap<K>.tbl once and is the only
   thing that needs it; `foldchk' then checks the folded reads against the
   plain table state by state. *)
...
let xpop = Array.init 4096 (fun s -> let c = ref 0 in for b = 0 to 11 do if s land (1 lsl b) <> 0 then incr c done; !c)
let xbitrank = Sys.getenv_opt "EXP_BITRANK" <> None
(* values of a.(off) .. a.(off+n-1) must be a permutation of their own range;
   they are made 0 .. n-1 by subtracting the minimum, which for the ranks used
   here is 0 (cp, ep outer) or 8 (ep middle) *)
let rankb (a : int array) off n base =
  let r = ref 0 and seen = ref 0 in
  for i = 0 to n - 1 do
    let v = a.(off + i) - base in
    let bv = 1 lsl v in
    r := !r * (n - i) + (v - xpop.(!seen land (bv - 1)));
    seen := !seen lor bv
  done; !r
let rank0 (a : int array) off n =
  let r = ref 0 in
  for i = 0 to n - 1 do
    let c = ref 0 in
...
let rank (a : int array) off n =
  if xbitrank then rankb a off n (if off = 8 && n = 4 then 8 else 0) else rank0 a off n
...
(* ---- the heuristic, folded by the same sixteen renamings --------------- *)

(* Phase one's distance is a distance by twist and by flip x slice, and a
   renaming sends a state to a state at the same distance, so one state of
   each orbit is enough.  The two coordinates fold apart from one another: a
   renaming sends a twist to a twist and a rank to a rank.

   All of it is lazy.  Only a row and the two fold modes want it, and the
   checks and the balls do not. *)

let nfs = n_flip * n_slice                      (* 1 013 760 *)

(* the cube of a rank: the flips of the one coordinate, the middle edges of
   the other *)
let cube_of_fs r =
  let c = cube_of_slice (r mod n_slice) in
  let e = cube_of_flip (r / n_slice) in
  for i = 0 to 11 do c.eo.(i) <- e.eo.(i) done;
  c

let fs_of c = flip c * n_slice + slice c

(* where a renaming sends a rank, and where it sends a twist *)
let fssym = lazy (
  Array.init nsym (fun s ->
    Array.init nfs (fun r -> fs_of (conj syms.(s) (cube_of_fs r)))))

let twsym = lazy (
  Array.init nsym (fun s ->
    Array.init n_twist (fun t -> twist (conj syms.(s) (cube_of_twist t)))))

(* The ranks gathered into orbits, as the pages are: one rank of each orbit
   is kept, the smallest, and every other rank is that one read through a
   renaming.  The renaming is SEARCHED, not inverted, so nothing about the
   sixteen is assumed here. *)
let fsfold = lazy (
  let sfs = Lazy.force fssym in
  let orb = Array.make nfs (-1) in          (* rank -> its orbit            *)
  let osym = Array.make nfs 0 in            (* rank -> renaming to the kept *)
  let kept = Array.make nfs 0 in            (* orbit -> the rank it keeps   *)
  let n = ref 0 in
  for r = 0 to nfs - 1 do
    if orb.(r) < 0 then begin
      kept.(!n) <- r;
      for s = 0 to nsym - 1 do
        let g = sfs.(s).(r) in
        if orb.(g) < 0 then orb.(g) <- !n
      done;
      incr n
    end
  done;
  for g = 0 to nfs - 1 do
    let k = kept.(orb.(g)) in
    let s = ref 0 in
    while !s < nsym && sfs.(!s).(g) <> k do incr s done;
    if !s = nsym then begin
      prerr_endline "a rank reaches no kept rank"; exit 1 end;
    osym.(g) <- !s
  done;
  (!n, Array.sub kept 0 !n, orb, osym))

(* THE THREE MOVES OF A FACE, IN FOUR BITS.  This is Rokicki's, and it is
   what makes an entry fit in four bytes.  Each move either brings the state
   nearer H, leaves it where it is, or takes it further, so a face would want
   twenty seven combinations; but U2 is U1 twice, so two moves of the same
   face cannot differ by more than one step, and a face that has a move
   bringing it nearer has none taking it further.  Twelve of the twenty seven
   go, fifteen are left, and fifteen fit in four bits.  Six faces and the
   distance are then twenty eight bits, where the moves written out one bit
   each are forty one. *)
let ncode = 15                                  (* codes for one face *)

(* the three effects of a face, -1 nearer, 0 the same, 1 further, run
   together as one number, and back *)
let trip e0 e1 e2 = ((e0 + 1) * 3 + (e1 + 1)) * 3 + (e2 + 1)

let codeof = Array.make 27 (-1)
let tripof = Array.make ncode 0
let () =
  let n = ref 0 in
  for e0 = -1 to 1 do
    for e1 = -1 to 1 do
      for e2 = -1 to 1 do
        ...
        if hi - lo <= 1 then begin
          codeof.(trip e0 e1 e2) <- !n; tripof.(!n) <- trip e0 e1 e2; incr n
        end
      done
    done
  done;
  if !n <> ncode then begin
    Printf.eprintf "the face codes are %d, not %d\n" !n ncode; exit 1 end

(* a code, as the three moves that bring the state nearer and the three that
   at least do not take it further *)
let dn3 = Array.make ncode 0
let fl3 = Array.make ncode 0
let () =
  for c = 0 to ncode - 1 do
    let t = tripof.(c) in
    let e k = (t / (if k = 0 then 9 else if k = 1 then 3 else 1)) mod 3 - 1 in
    for k = 0 to 2 do
      if e k < 0 then dn3.(c) <- dn3.(c) lor (1 lsl k);
      if e k <= 0 then fl3.(c) <- fl3.(c) lor (1 lsl k)
    done
  done

(* THE MOVES ARE THE KEPT STATE'S, so they have to be renamed back before
   they are played.  Decoding and renaming are one table apiece: three faces
   at a time, twelve bits in, the eighteen moves of the state itself out. *)
let imv = Array.init nsym (fun s ->
  let a = Array.make 18 0 in
  for m = 0 to 17 do a.(symmv.(s).(m)) <- m done; a)

let mktab dec half = Array.init nsym (fun y ->
  Array.init 4096 (fun w ->
    let r = ref 0 in
    for i = 0 to 2 do
      (* a four bit field holds sixteen values and only fifteen are codes;
         the sixteenth never occurs in the table and stands for no move *)
      let c = (w lsr (4 * i)) land 15 in
      if c < ncode then begin
        let b = dec.(c) in
        for k = 0 to 2 do
          if (b lsr k) land 1 = 1 then
            r := !r lor (1 lsl imv.(y).(3 * (half + i) + k))
        done
      end
    done;
    !r))

let dnlo = mktab dn3 0
let dnhi = mktab dn3 3
let fllo = mktab fl3 0
let flhi = mktab fl3 3
...
let xnowrite = Sys.getenv_opt "EXP_NOWRITE" <> None
let xsink = ref 0
let xmlev = match Sys.getenv_opt "EXP_MLEV" with Some v -> int_of_string v | None -> 9
let fmark_at cp ep =
  if xmlev = 0 then xsink := !xsink + cp.(0) + ep.(0) else
  let pg = rank cp 0 8 in
  if xmlev = 1 then xsink := !xsink + pg else
...
let lrow n =
  ...
  if not (Sys.file_exists path) then begin
    prerr_endline "phase1_cap9.tbl is not there: ./rubik_row_fold 9 20 build";
    exit 1 end;
...
let () =
  ...
  if Array.length Sys.argv > 2 && Sys.argv.(1) = "dumptab" then begin
    ...
    Buffer.add_string out
      "(* GENERATED by ocaml/rubik_row_fold.ml dumptab -- do not edit.        \
       \     *)\n\
  ...
  (* The plain pruning table, one byte a state, shared by every worker
     through the file system.  IT IS LAZY: the folded table is built out of
     it and checked against it, and a row never opens it at all. *)
  let n_all = n_twist * n_flip * n_slice in
  let path = Printf.sprintf "phase1_cap%d.tbl" cap in
  let plain = lazy begin
  ...
  p_all end in
  (* THE FOLDED TABLE.  A word a KEPT state: the distance, the moves that
     drop it by one, and the moves that do not raise it.  64 430 kept ranks
     against 1 013 760, so 140 918 610 words, 1.13 GB, where writing every
     state out costs 17.7 GB.

     The sweep runs twist by twist because that is the order the plain table
     is laid out in: the eighteen children of a twist sit in eighteen rows
     of 1 013 760 bytes, and the kept ranks walk each row forwards. *)
  let (norb, fkept, forb, fosym) = Lazy.force fsfold in
  let ftw = Lazy.force twsym in
  let nfold = norb * n_twist in
  let fpath = Printf.sprintf "phase1f%d.tbl" cap in
  let want_fold = (arg = "fold") in
  let want_fchk = (arg = "foldchk") in
  let has_fold = Sys.file_exists fpath in
  (* Cap 12 is not a soundness matter -- the masks are right at any cap,
     ...
     lower one would cost the same 1.13 GB and prune worse. *)
  if want_fold && cap < 12 then
    prerr_endline "note: the cap does not change the size, so a table below \
12 costs as much and prunes weaker -- it is worth building only to check \
the fold";
  if want_fold && has_fold then begin
    Printf.printf "%s is already there\n%!" fpath; exit 0 end;
  if not (want_fold || has_fold) && not build_only then begin
    Printf.eprintf "%s is not there: build it with `%d %d fold'\n" fpath
      cap maxdepth;
    exit 1 end;
  (* four bytes an entry, so a file of the wrong length is a file from
     another layout and is refused rather than read *)
  if has_fold && (Unix.stat fpath).Unix.st_size <> nfold * 4 then begin
    Printf.eprintf "%s is %d bytes, not %d: it is from another layout\n"
      fpath (Unix.stat fpath).Unix.st_size (nfold * 4);
    exit 1 end;
  let p_fld =
    if build_only then
      Bigarray.Array1.create Bigarray.int32 Bigarray.c_layout 0
    else begin
      let fd = Unix.openfile fpath
          (if want_fold then [Unix.O_RDWR; Unix.O_CREAT] else [Unix.O_RDONLY])
          0o644 in
      let a = Bigarray.array1_of_genarray
          (Unix.map_file fd Bigarray.int32 Bigarray.c_layout want_fold
             [| nfold |]) in
      Unix.close fd; a
    end in
  if want_fold then begin
    let p_all = Lazy.force plain in
    Printf.printf "building the folded table (%d entries, %.2f GB)...\n%!"
      nfold (float_of_int nfold *. 4.0 /. 1e9);
    let t0 = Unix.gettimeofday () in
    let base = Array.make 18 0 in
    for t = 0 to n_twist - 1 do
      let mtt = mt_twist.(t) in
      for m = 0 to 17 do base.(m) <- mtt.(m) * nfs done;
      let here = t * nfs and out = t * norb in
      for o = 0 to norb - 1 do
        let r = Array.unsafe_get fkept o in
        let mtf = Array.unsafe_get mt_flip (r / n_slice)
        and mts = Array.unsafe_get mt_slice (r mod n_slice) in
        let d = Char.code (Bigarray.Array1.unsafe_get p_all (here + r)) in
        let w = ref d in
        for fc = 0 to 5 do
          let e k =
            let m = 3 * fc + k in
            let j = Array.unsafe_get base m
                    + Array.unsafe_get mtf m * n_slice
                    + Array.unsafe_get mts m in
            let dc = Char.code (Bigarray.Array1.unsafe_get p_all j) in
            if dc < d then -1 else if dc = d then 0 else 1 in
          let c = codeof.(trip (e 0) (e 1) (e 2)) in
          if c < 0 then begin
            prerr_endline "a face moves the distance both ways"; exit 1 end;
          w := !w lor (c lsl (4 + 4 * fc))
        done;
        Bigarray.Array1.unsafe_set p_fld (out + o) (Int32.of_int !w)
      done;
      if t land 255 = 0 then
        Printf.printf "   twist %d of %d (%.0f s)\n%!" t n_twist
          (Unix.gettimeofday () -. t0)
    done;
    Printf.printf "the folded table took %.0f s\n%!"
      (Unix.gettimeofday () -. t0);
    exit 0
  end;
  if build_only then begin ignore (Lazy.force plain); exit 0 end;

  (* A state, folded: its rank names the orbit and the renaming that carries
     the rank to the orbit's kept one, and the twist is played through that
     same renaming.  The renaming is carried back in the top bits of the
     word, because the moves the word names are the KEPT state's. *)
  let fword t f s =
    let r = f * n_slice + s in
    let y = Array.unsafe_get fosym r in
    Int32.to_int (Bigarray.Array1.unsafe_get p_fld
      (Array.unsafe_get (Array.unsafe_get ftw y) t * norb
       + Array.unsafe_get forb r))
    lor (y lsl 32) in

  (* A word of the folded table: four bits of distance, then six faces of
     four, and last the renaming it was read through.  A move changes the
     distance by at most one, so a node with s moves to spare beyond its
     distance may take: any move if s is two or more, a move that does not
     take it further if s is one, and only a move that brings it nearer if s
     is nought.  Decoding and renaming are one table read a half. *)
  let allmv = (1 lsl 18) - 1 in
  let mdist w = w land 15 in
  let mmask w s =
    if s >= 2 then allmv
    else begin
      let y = (w lsr 32) land 15 in
      let c = w lsr 4 in
      if s = 1 then
        Array.unsafe_get (Array.unsafe_get fllo y) (c land 4095)
        lor Array.unsafe_get (Array.unsafe_get flhi y) ((c lsr 12) land 4095)
      else
        Array.unsafe_get (Array.unsafe_get dnlo y) (c land 4095)
        lor Array.unsafe_get (Array.unsafe_get dnhi y) ((c lsr 12) land 4095)
    end in

  (* ---- the folded table, checked against the plain one ----------------- *)
  (* Four checks, and the last two are the ones that matter: a folded read
     must give the plain distance, and the moves it names must be the moves
     the plain table names, once they are renamed back. *)
  if want_fchk then begin
    let p_all = Lazy.force plain in
    let nchk =
      if Array.length Sys.argv > 4 then int_of_string Sys.argv.(4)
      else 200000 in
    let sfs = Lazy.force fssym in
    Printf.printf "%d kept ranks of %d (fold %.2fx), %d words\n%!"
      norb nfs (float_of_int nfs /. float_of_int norb) nfold;
    (* each renaming is a permutation of the ranks and of the twists *)
    let bad = ref 0 in
    let hit = Array.make nfs false in
    for y = 0 to nsym - 1 do
      Array.fill hit 0 nfs false;
      Array.iter (fun v -> if hit.(v) then incr bad else hit.(v) <- true)
        sfs.(y)
    done;
    let hitw = Array.make n_twist false in
    for y = 0 to nsym - 1 do
      Array.fill hitw 0 n_twist false;
      Array.iter (fun v -> if hitw.(v) then incr bad else hitw.(v) <- true)
        ftw.(y)
    done;
    Printf.printf "renamings that are not permutations: %d\n%!" !bad;
    (* renaming then moving is moving then renaming, the move renamed too *)
    let badw = ref 0 in
    let stride = 97 in
    for y = 0 to nsym - 1 do
      let r = ref 0 in
      while !r < nfs do
        let f = !r / n_slice and sl = !r mod n_slice in
        for m = 0 to 17 do
          let rm = mt_flip.(f).(m) * n_slice + mt_slice.(sl).(m) in
          let g = sfs.(y).(!r) in
          let m' = symmv.(y).(m) in
          let gm = mt_flip.(g / n_slice).(m') * n_slice
                   + mt_slice.(g mod n_slice).(m') in
          if sfs.(y).(rm) <> gm then incr badw
        done;
        r := !r + stride
      done;
      for t = 0 to n_twist - 1 do
        for m = 0 to 17 do
          if ftw.(y).(mt_twist.(t).(m))
             <> mt_twist.(ftw.(y).(t)).(symmv.(y).(m)) then incr badw
        done
      done
    done;
    Printf.printf "renamings that do not commute with the moves: %d\n%!" !badw;
    (* the folded distance is the plain one, and the folded moves are the
       plain ones renamed back *)
    let st = Random.State.make [| 20260825 |] in
    let badd = ref 0 and badm = ref 0 in
    let t0 = Unix.gettimeofday () in
    for _ = 1 to nchk do
      let t = Random.State.int st n_twist
      and f = Random.State.int st n_flip
      and sl = Random.State.int st n_slice in
      let w = fword t f sl in
      let d = Char.code (Bigarray.Array1.unsafe_get p_all
                           ((t * n_flip + f) * n_slice + sl)) in
      if mdist w <> d then incr badd;
      let down = ref 0 and flat = ref 0 in
      for m = 0 to 17 do
        let dc = Char.code (Bigarray.Array1.unsafe_get p_all
            ((mt_twist.(t).(m) * n_flip + mt_flip.(f).(m)) * n_slice
             + mt_slice.(sl).(m))) in
        if dc < d then down := !down lor (1 lsl m);
        if dc <= d then flat := !flat lor (1 lsl m)
      done;
      if mmask w 0 <> !down || mmask w 1 <> !flat then incr badm
    done;
    Printf.printf "%d states: %d wrong distances, %d wrong masks (%.1f s)\n%!"
      nchk !badd !badm (Unix.gettimeofday () -. t0);
    if !bad > 0 || !badw > 0 || !badd > 0 || !badm > 0 then begin
      prerr_endline "THE FOLDED TABLE IS WRONG"; exit 1 end;
    print_endline "the folded table agrees with the plain one";
    exit 0
  end;
  ...
  (* HOW WIDE A NODE IS.  A node pays a coordinate step and a table read for
  ...
  let fld d = fword tw.(d) fl.(d) sl.(d) in
  let heur d = mdist (fld d) in

  let xnostep = Sys.getenv_opt "EXP_NOSTEP" <> None in
  let xlate = Sys.getenv_opt "EXP_LATE" <> None in
  let xlast = Sys.getenv_opt "EXP_LAST" <> None in
  let stepp d m =
    let d' = d + 1 in
    let mcp = moves.(m).cp and mep = moves.(m).ep in
    for i = 0 to 7 do cps.(d').(i) <- cps.(d).(mcp.(i)) done;
    for i = 0 to 11 do eps.(d').(i) <- eps.(d).(mep.(i)) done in
  let xnomark = Sys.getenv_opt "EXP_NOMARK" <> None in
  let xnocnt = Sys.getenv_opt "EXP_NOCNT" <> None in
  let step d m =
    let d' = d + 1 in
    let mcp = moves.(m).cp and mep = moves.(m).ep in
    if not xnostep && not xlate then begin
    for i = 0 to 7 do cps.(d').(i) <- cps.(d).(mcp.(i)) done;
    for i = 0 to 11 do eps.(d').(i) <- eps.(d).(mep.(i)) done end;
  ...
  (* THE SEARCH.  A node is handed the moves worth trying, so it never builds
     a position the table has already ruled out -- the eighteen tries and
     eighteen table reads a node become three or four of each.
  ...
  let rec dfsm d togo prev mask =
    if not xnocnt then nodes := Int64.add !nodes 1L;
    if !nset >= !enough then ()
    else if togo = 0 then begin
      if not xnocnt then probes := Int64.add !probes 1L;
      if not xnomark then mark d
    end else begin
      if not xnocnt then begin
      inner := Int64.add !inner 1L;
      widths := Int64.add !widths (Int64.of_int (popc18 mask));
      if mask = allmv then wide := Int64.add !wide 1L end;
      let togo' = togo - 1 in
      if xlast && togo = 1 then begin
        (* hcoset's last level: the mask names the moves that land in H, so
           the children are marked with no step of the summary and no read *)
        for m = 0 to 17 do
          if mask land (1 lsl m) <> 0 then begin
            let f = m / 3 in
            if (not !cut || not ish.(m))
               && (prev < 0 || not (f = prev || (f = opp prev && f > prev)))
            then begin
              stepp d m;
              if !nset < !enough && not xnomark then mark (d + 1)
            end
          end
        done
      end else
      for m = 0 to 17 do
        if mask land (1 lsl m) <> 0 then begin
          ...
          if (togo > 1 || not !cut || not ish.(m))
             && (prev < 0 || not (f = prev || (f = opp prev && f > prev)))
          then begin
            if not xnocnt then tries := Int64.add !tries 1L;
            step d m;
            let w = fld (d + 1) in
            let nd = mdist w in
            if nd <= togo'
               && (not !cut || togo' = nd || togo' + nd >= rcut) then begin
              if xlate then stepp d m;
              dfsm (d + 1) togo' f (mmask w (togo' - nd)) end
          end
        end
      done
    end in
  ...
  if (rowplace <> None || rowsolve || rowleft >= 0)
    ...
    let fld1 d = fword tws.(d) fls.(d) sls.(d) in
    let step1 d m =
      let d' = d + 1 in
      let mcp = moves.(m).cp and mep = moves.(m).ep in
      for i = 0 to 7 do cps.(d').(i) <- cps.(d).(mcp.(i)) done;
    ...
    let d1 c = mdist (fword (twist c) (flip c) (slice c)) in

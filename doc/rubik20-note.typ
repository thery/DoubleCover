...
After $U$, repetition removes $U$, $U^2$ and $U^(-1)$, which leaves fifteen. The
order convention would remove $D$, $D^2$ and $D^(-1)$ as well and leave twelve,
but it may not be used here: the first move is already fixed to the top face,
and turning the cube upside down takes $D U$ back to $U D$. So the bottom face
stays, and the fifteen second moves after $U$ include $U D$, $U D^2$ and
$U D^(-1)$.

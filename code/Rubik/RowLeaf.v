...
(* memb2tab builds a cube from three ranks and tomemb reads three ranks off   *)
(* a cube; leaf_memb and tomemb_tab say the two undo each other on a          *)
(* position of H.                                                             *)
(*                                                                            *)
(* The premise is that the position is in H -- no corner twisted, no edge     *)
(* flipped, the middle four in the middle layer -- and not anything about     *)
(* the pruning table, which soundness never reads.  Being in H says the same  *)
(* thing once for each layout: the position carries a place to a place and    *)
(* leaves the slot alone.  That is assumed here as hqc, hqu and hqm.          *)
...
(* later places hold something smaller.  That is the Lehmer digit lcode       *)
(* below, so the parity and the rank count the same thing.  Counting it this  *)
(* way makes exchanging two neighbouring places a local fact: every digit     *)
(* but two is untouched.                                                      *)

(* ---- inversions are the sign, and that has to be proved once ------------- *)

(* mathcomp's odd_perm counts orbits and prmn counts inversions.  The two     *)
(* agree, and the library does not say so, so the bridge is proved here.      *)
(* odd_perm cannot be used instead: prmn has to compute, for the parity       *)
(* table walk, and a permutation of ordinals does not.                        *)

(* the identity inverts nothing                                               *)

(* The classical fact: exchanging two neighbouring places changes the number  *)
(* of inversions by exactly one.  The pairs fall into four kinds:             *)
(*                                                                            *)
...
(*   one place is i + 1, other before   statuses, so the count is unchanged   *)
(*                                                                            *)
(* Written out it is a reindexing of the count over the pairs by the          *)
(* exchange, which is an involution on them.  Every permutation is reached    *)
(* from the identity by such exchanges, so prmn_mul follows by induction.     *)
(* The premise q i <> q (i + 1) is needed: a constant q inverts nothing       *)
(* either way, and this is where injectivity is used.                         *)
...
(* At the two places the count moves by one: the digit at i becomes the one   *)
(* that was at i + 1 plus whether q i is below q (i + 1), and the digit at    *)
(* i + 1 the one that was at i less whether q (i + 1) is below q i.  A        *)
(* permutation makes exactly one of those hold.                               *)
...
(* places, so it never reaches n factorial.  True of any function.            *)

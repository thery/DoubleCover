...
rewrite {3}/fexp.
set mg := (mag _ _) -p .
case:(ZArith_dec.Z_lt_le_dec   mg emin)=> h; last first.
  rewrite Zmax.Zmax_left; last first.  rewrite Z.max_l // /mg.  lia.
  move=>Er.
  ...
  apply:(bpow_le beta 0); apply Zorder.Zle_minus_le_0; rewrite /cexp {1 4}/ fexp -/(ce x) Er'; lia.
rewrite (Z.max_r _ emin); last lia.
set mg' := (mag _ _ )%Z.

case:(ZArith_dec.Z_lt_le_dec   mg' emin)=> h'; last first. 
 rewrite Zmax.Zmax_left; last  by   lia.
  move=>Er.
  have: (mag beta (Rabs x) = mag beta (pow (mag beta (m * pow (ce x)))) :>Z) 
    by rewrite Er.
  rewrite  mag_bpow mag_abs=> Er'.
...
have: (cond_Ropp (Rlt_bool x 0) (pow (emin  + - ce x)) *
 ...
 rewrite Er'; lia.
  ...
  have H' := (sym_eq H).
  rewrite (@Znearest_scale ( pow (ce x + - ce (m * pow (ce x)))) m zx) //.
  apply:(bpow_le beta 0); apply Zorder.Zle_minus_le_0; 
...
apply/Z.max_lub.
  apply( Zorder.Zplus_le_reg_l _ _ p); ring_simplify.
...
(* 1. *)
  ...
  rewrite -/Ma -/mu.
  case/ZArith_dec.Z_le_lt_eq_dec; last first.
...
case: (Zorder.Zle_or_lt es eb) => [Hle|Hlt].
(* Hle : es <= eb *)
  have sExact: (s = a + b)%R.
    rewrite round_generic //.

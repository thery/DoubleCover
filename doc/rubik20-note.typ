...
records, for each summary, its distance from the solved summary. The generator
stops at nine and stores ten for everything further away. That is still a lower
bound, which is all the search asks of it, and it keeps an entry to four bits,
fifteen to a machine word: the whole table is _1.18 GB_. The cut is quite effective. A
...
The table is generated by an OCaml program, and up to nine it gives the exact
distance. 

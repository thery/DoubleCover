...
Definition actcd (x : int) (k : nat) : int :=
  actf x (nth (mdatf_of_tab [::]) (mdataf mtabs) k).
...
congr (actf _ _).
by rewrite /mdataf (nth_map [::]) ?hmt // -ti2t_mtis seq.size_map.
Qed.
...
Lemma far_of_searchic d a :
  tabi_ok 47 a -> cubti a ->
  searchic 47 mtis nfcube oppf fcpos Dfsd actcd d a (coordi a) nfcube = false ->
  pt 47 (ti2t 47 a) \notin ball Sset d.
Proof.
move=> aok ca sE; apply: (far_of_searchir aok).
rewrite -(searchicE n47_small n47_len mtis_ok nfcube oppf fcpos
                    DtidE2 cubti_comp actcdE d nfcube aok ca).
exact: sE.
Qed.

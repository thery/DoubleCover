(* =========================================================================  *)
(*  RowPrep.v -- the cell of forty eight bits, from the place of twenty four.*)
(* =========================================================================  *)
...
Lemma bit_lo24 v i : (i <? nbiti)%uint63 ->
  bit (Uint63.land v allbits24) i = bit v i.
Proof.
...
Lemma bit_hi24 v s : (nbiti <=? s)%uint63 -> (s <? nbit48i)%uint63 ->
  bit (Uint63.land (Uint63.lsr v nbiti) allbits24) (Uint63.sub s nbiti) = bit v s.
Proof.
...
Hypothesis grpmv24P : forall k v bt', (to_nat k < nhn)%N ->
  (bt' <? nbiti)%uint63 ->
...
Definition halfw (v h : int) : int :=
  if Uint63.eqb h 0%uint63 then Uint63.land v allbits24
  else Uint63.land (Uint63.lsr v nbiti) allbits24.

Lemma halfw0 v : halfw v 0%uint63 = Uint63.land v allbits24.
Proof. by rewrite /halfw eqb_refl. Qed.

Lemma halfw1 v :
  halfw v 1%uint63 = Uint63.land (Uint63.lsr v nbiti) allbits24.
...
Lemma from_half k v h j bt' : (to_nat k < nhn)%N -> (to_nat h < 2)%N ->
  ...
  ~~ (Uint63.land (grpmv24 msw mlo mhi k (halfw v h)) (bitof j) =? 0)%uint63 ->
  exists2 bt, (bt <? nbit48i)%uint63 &
    btmv48 k bt = bt' /\ ~~ (Uint63.land v (bitof bt) =? 0)%uint63.
Proof.
move=> hk hh hj hE hset.
have [bt0 hbt0 [hmv hin]] := grpmv24P hk hj hset.
...
Lemma bit_mask24 x j :
  bit (Uint63.land x allbits24) j = bit x j && (j <? nbiti)%uint63.
Proof.
...
Lemma split_lor48 a b bt' : (bt' <? nbit48i)%uint63 ->
  bit (Uint63.lor (Uint63.land a allbits24)
                  (Uint63.lsl (Uint63.land b allbits24) nbiti)) bt' ->
...
rewrite test_bit // /grpmv in hset.
have e0 : Uint63.land v allbits24 = halfw v 0%uint63 by rewrite halfw0.
have e1 : Uint63.land (Uint63.lsr v nbiti) allbits24 = halfw v 1%uint63.
  by rewrite halfw1.
...
Lemma unplace48E pg gr bt : (pg <? nclsi)%uint63 -> (bt <? nbit48i)%uint63 ->
  ...
  = unplace24 e8inv e4of par8 par4 (pgof pg bt) gr (Uint63.mod bt nbiti).
Proof.
move=> hpg hbt; rewrite /unplace /unplace24 /pgof.
by have [_ ->] := pgof_page hpg hbt.
Qed.

Lemma inrange48E pg gr bt : inrange pg gr bt ->
  inrange24 (pgof pg bt) gr (Uint63.mod bt nbiti).
Proof.
...
Definition cpgok481 (k : int) : bool :=
  iter nclsn 0%uint63 (fun pg =>
    iter 2 0%uint63 (fun s =>
      ...
      (PArray.get e8inv
         (Uint63.add (Uint63.mul (pgmv cpg k pg) 2%uint63)
...
Hypothesis prep_move24 : forall k pg gr bt, (to_nat k < nhn)%N ->
  inrange24 pg gr bt ->
  inrange24 (pgmv mpg k pg) (grmv mgr k gr) (btmv k bt) /\
  pos (unplace24 e8inv e4of par8 par4
         (pgmv mpg k pg) (grmv mgr k gr) (btmv k bt))
  = pos (unplace24 e8inv e4of par8 par4 pg gr bt) * hmv k.
...
Lemma prep_move48 k pg gr bt : (to_nat k < nhn)%N -> inrange pg gr bt ->
  inrange (pgmv cpg k pg) (grmv mgr k gr) (btmv48 cfl btmv k bt) /\
  pos (unplace e8inv e4of par8 par4 (pgmv cpg k pg) (grmv mgr k gr)
...
have hr24 : inrange24 (pgof pg bt) gr (Uint63.mod bt nbiti) := inrange48E hr.
have [hr24' hpos] := prep_move24 hk hr24.
...
have hpgof : pgof (pgmv cpg k pg) (btmv48 cfl btmv k bt)
...
Lemma inrange48_conv pg gr bt : inrange24 pg gr bt ->
  inrange (Uint63.div (PArray.get e8num pg) 2%uint63) gr
            (Uint63.add (Uint63.mul (PArray.get par8 pg) nbiti) bt).
Proof.
...
Lemma unplace48_conv pg gr bt : inrange24 pg gr bt ->
  unplace e8inv e4of par8 par4
    (Uint63.div (PArray.get e8num pg) 2%uint63) gr
    (Uint63.add (Uint63.mul (PArray.get par8 pg) nbiti) bt)
  = unplace24 e8inv e4of par8 par4 pg gr bt.
Proof.

...
// Names in monospace link to their declaration on GitHub.  The table is
// generated by doc/mklinks.py, which must be re-run when the sources move.
#import "lefevre-links.typ": srclinks
#let name(x) = if x in srclinks {
  link(srclinks.at(x), text(fill: rgb("#0b5394"), raw(x)))
} else { raw(x) }

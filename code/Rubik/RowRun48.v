(* =========================================================================  *)
(*  RowRun48.v -- the same run, over a map of forty eight bit cells.         *)
(* =========================================================================  *)
...
(*                                                                            *)
(* Every cut here is safe, which is why a row is cheaper to prove than a      *)
(* lower bound: what is proved is that the map filled, not that the search    *)
(* was complete.  A cut that loses words can only make the row finish later   *)
(* or not at all, never call a member covered when it is not, so nothing      *)
(* below asks the search for completeness.                                    *)
(*                                                                            *)
...
(*                                                                            *)
(* THIS IS RowRun.v OVER Row48's LAYOUT, and it is a file of its own because  *)
(* RowRun.v names place and unplace directly.  Editing that file would move   *)
(* its .vo and every run banked on it, so it is left alone.  Line for line    *)
(* the two are the same argument: what changed is the layout it stands on,    *)
(* the width of a cell, and that a page is a corner PAIR -- and a pair number *)
(* is a page number, which is the only bridge the proofs needed.              *)
...
Local Notation prep := (prepass48 cpg cfl mgr msw mlo mhi).
Local Notation prepm := (prepmv48 cpg cfl mgr msw mlo mhi).
Local Notation pgm := (cpgmv cpg).
Local Notation grm := (grmv mgr).
Local Notation grpm := (grpmv48 cfl msw mlo mhi).
...
(* The table holds the distance alone, four bits, so there is no mask to      *)
(* read and every move is offered.  That costs breadth and nothing else: a    *)
(* mask that offers too much makes the search bigger, never wrong.            *)
Definition wmask (w : int) (s : nat) : int := allmv.

(* The search carries a position, not a member: a member is what it reaches   *)
(* when the coordinate is solved, and tomemb reads it off at a leaf.  posp    *)
(* is the word played -- the row's representative undone and the carried      *)
(* position put back -- so at the root it is the identity and a move takes    *)
(* it one step further out.                                                   *)
Variable pst : Type.
...
Definition soundat (m : rmap) (d : nat) : Prop :=
  forall pg gr bt,
    inrange48 pg gr bt -> mtest m pg gr bt -> wthn d (unplc pg gr bt).
...
Hypothesis prep_move : forall k pg gr bt, (to_nat k < nhn)%N ->
  inrange48 pg gr bt ->
  inrange48 (pgm k pg) (grm k gr) (btmv k bt) /\
...
have hri : inrange48 pg gr bt.
  by rewrite /inrange48 hbt !andbT; apply/andP; split; apply/nltbP.
...
Proof.
move=> hk hm hdst; rewrite /prepmv48.
(* every page                                                                 *)
...
Lemma prepmv0_sound k src dst d : (to_nat k < nhn)%N ->
  soundat src d -> soundat dst d.+1 -> soundat (prepmv048 cpg cfl mgr msw mlo mhi
    k src dst) d.+1.
Proof.
move=> hk hm hdst; rewrite /prepmv048.
apply: (@ifold_indi _ (fun a => soundat a d.+1)); [| |exact: hdst].
  by apply: ltnW; exact: nclsn_nwB.
move=> pg a hpg ha; cbv zeta.
apply: (@ifold_indi _ (fun a' => soundat a' d.+1)); [| |exact: ha].
...
by rewrite /inrange48; apply/and3P; split;
...
elim: togo c x msk pv m => [|togo ih] c x msk pv m hdt hc hp hnd hm hb.
  ...
  have E : plc (tomemb x) =
      (Uint63.div (PArray.get e8num (mcp (tomemb x))) 2%uint63,
       ...
       Uint63.add (Uint63.mul (PArray.get par8 (mcp (tomemb x))) nbiti)
                  (PArray.get e4bit (mmp (tomemb x)))) by [].
  move=> pg' gr' bt' hr ht.
  case: (mmark48P (place48_range he8 he4 hok E) hr ht) => [[<- <- <-]|hb2];
    last by apply: hm.
  rewrite /wthn (unplace48_place48 he8 he4 hok E) (leaf_pos hc hp hG hs).

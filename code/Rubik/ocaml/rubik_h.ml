...
let () =
  ...
  (* ---- the small tables, as Rocq: no table needed either ----------------- *)

  (* Beside the big table the Rocq search needs the three move tables and the
     two maps of the fold, which come to 2.6 million entries: the moves on e,
     on cl and on ct, then which symmetry takes an e to the least of its
     family and which family that is, and how the sixteen symmetries act on
     cl and on ct.  All of it is computed here from nothing.

     Row major with twelve columns, so a step is one multiplication and one
     read.  The largest is the one on e, 190080 * 12 = 2 280 960 entries,
     which is inside PArray's limit of 4 194 303.                           *)
  if mode = "mtabs" then begin
    let sym_cl, sym_ct, rep, which, cls, _ = families () in
    let emit name n get =
      Printf.printf "Definition %s : array int := [|\n" name;
      for i = 0 to n - 1 do
        Printf.printf "%d%s" (get i)
          (if i = n - 1 then "" else if i mod 15 = 14 then ";\n" else "; ")
      done;
      Printf.printf "\n| 0 |].\n\n" in
    Printf.printf
      "(* GENERATED by ocaml/rubik_h.ml -- do not edit.                    *)\n\
       (* The move tables of Reid's three coordinates, and the two maps    *)\n\
       (* of the fold by the sixteen symmetries.                          *)\n\n\
       From Stdlib Require Import Uint63.\n\
       From Stdlib Require Import PArray.\n\n\
       Local Open Scope uint63_scope.\n\n";
    emit "h_mt_e"  (n_e  * nmv) (fun k -> mt_e.(k / nmv).(k mod nmv));
    emit "h_mt_cl" (n_cl * nmv) (fun k -> mt_cl.(k / nmv).(k mod nmv));
    emit "h_mt_ct" (n_ct * nmv) (fun k -> mt_ct.(k / nmv).(k mod nmv));
    emit "h_which" n_e (fun e -> which.(e));
    emit "h_fam"   n_e (fun e -> cls.(rep.(e)));
    emit "h_sym_cl" (nsym * n_cl) (fun k -> sym_cl.(k / n_cl).(k mod n_cl));
    emit "h_sym_ct" (nsym * n_ct) (fun k -> sym_ct.(k / n_ct).(k mod n_ct));
    exit 0
  end;

(* =========================================================================  *)
(*  RowCubReal48.v -- the row on the PLAIN map, with nothing left open but the *)
(*  run.                                                                     *)
(* =========================================================================  *)
...
Require Import RowSrchP48 RowMark48 RowCubInst48 RowTab48 RowTabP48.
...
Hypothesis prep_eq : forall m dst,
  prep m dst = prepass48 cpgi cfli mgri mswi mloi mhii m dst.
...
Definition ycmfinsp : rmap :=
  ymfinsk e8numi e4biti par8i F frep fsym twsym dnlo dnhi fllo flhi
          ishm prep actfsri tomembi okmvv RowReal.srch 20.
...
exact: (ymfinsk_sound e8okC e4okC prep_eq memb2tab_okC srcokC halfokC
          ...
          pgokC grokC btokC cflokC cpgok48C memb2tab_moveC).
Qed.
...
Lemma pwits_sound l m :
  wgood48 e8invi e4ofi par4i (RowInst.ptab memb2tab) l ->
  RowRun48.soundat e8invi e4ofi par4i
...
Theorem real_superflip_row_p : mfull48 ycwitsr ->
  forall h, h \in H -> superflip^-1 * h \in ball Sset 20.
Proof.
move=> hf h hh.
have [x [hx hxe]] :=
  row_cover up8invC up8okC up4invC up4okC par8okwC par4okwC hh.
case E : (place48 e8numi e4biti par8i x) => [[pg gr] bt].
have hr := place48_range e8okC e4okC hx E.
have hu := unplace48_place48 e8okC e4okC hx E.
have := ycwitsr_sound hr (mfull48P hf hr).

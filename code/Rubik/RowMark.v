(* =========================================================================  *)
(*  RowMark.v -- the witnesses marked into the map, instead of a map of their *)
(*  own.                                                                      *)
(* =========================================================================  *)

(* RowFinal keeps the witnesses in a SECOND map and asks mfull2 of the two.   *)
...
(* witness is a member within twenty, and a mark keeps a map sound.  mfull    *)
...
Local Notation wgood := (RowFinal.wgood e8inv e4of par8 par4 ptab).
...
by case: (mmarkP hr hr' ht') => [[<- <- <-]|hb]; [exact: hw | exact: hm].
Qed.
...
(* RowFinal.row_within_20, with mfull of the marked map where it had mfull2   *)
(* of two.  The witnesses enter through wmark_sound instead of wmap_wit.      *)
Variable mfin : rmap.

Theorem row_within_20_marked :
  soundat mfin 20 ->
  RowFinal.wgood e8inv e4of par8 par4 ptab wl ->
  mfull (wmarkof wl mfin) ->
...
rewrite -hu.
by apply: (wmark_sound hw hs) => //; exact: (mfullP hf hr).
Qed.

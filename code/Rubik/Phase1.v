...
(*  1.  The corner data                                                       *)
(*                                                                            *)
(*  Derived from the move tables, not read off a picture.  A facelet is a     *)
(*  corner facelet iff f mod 8 is in {0,2,5,7}, and two of them sit on the    *)
(*  same cubie iff the same face turns fix them.                              *)
(*                                                                            *)
(*  The U/D axis must be the one the flip and slice coordinate uses, namely   *)
(*  the two faces whose quarter turn leaves the slice alone.  Note that the   *)
(*  move group k / 3 and the facelet block f / 8 do not agree: move group 3   *)
(*  turns block 5.  The U/D groups are 0 and 3, the U/D blocks 0 and 5, and   *)
(*  it is the blocks that carry the stickers below.                           *)
(* =========================================================================  *)
...
(* The three stickers of each corner, the U/D one first.  The cyclic order of *)
(* the other two is not free: orientation is an action only when the 3-cycle  *)
(* turning every corner in place commutes with every move, which is cubcP.    *)
(* Two of the 256 masks qualify, the two chiralities; this is the smaller.    *)
...
(* Three levels, as Coordfs and Coordfsi do for flip and slice: arrays for    *)
(* the search, lists between, permutations for the proofs.  ectwistiE and     *)
(* ctwisttE are the bridges.  In all three u is the inverse, so u at slot s   *)
(* is the facelet occupying s.                                                *)
...
(* acttw is computed from the move, as Coordfs.actfs is, and is not a         *)
(* generated table, so coordtw_step is a statement about the cube and not     *)
(* about emitted numbers.  A corner facelet belongs to a cubie (cpos) and     *)
(* sits in one of its three slots (cslot), the U/D slot being slot 0; a move  *)
(* sends the U/D slot of corner p to some slot of some corner, and that pair  *)
(* is the action on the digit.                                                *)
...
(*                                                                            *)
(*  Everything here is on nat: the values are below 3 ^ 7 = 2187, so the      *)
(*  int63 side is reached once at the end rather than carried throughout.     *)
(* =========================================================================  *)
...
(* The computation side.  {perm facelet} is a 48 element finfun that no       *)
(* evaluator can touch, so the check runs this 2187 x 18 table instead, and   *)
(* acttwiE ties the two together over its 39 366 entries.                     *)
...
(* The move index as an int63, so the inner loop does not run of_nat on it    *)
(* eighteen times a node.  acttwi is defined from acttwii rather than         *)
(* repeating its body: with two copies, acttwiiE closes by evaluating twmove. *)
...
(* Both loop invariants are lifted out, or the check does not run: acttwt as  *)
(* written rebuilds inv_tab 47 mt for each digit and each of the 2187         *)
(* twists, and dign divides a unary nat.  Same shape as p1mdata in section 5. *)
...
(* An equation, not a delta step.  Unfolding acttwiCk in place does not       *)
(* return; going through this lemma is instant.  The conversion is free, so   *)
(* what is slow is the unification, not the kernel.  See p1checkTwE below.    *)
...
(*                                                                            *)
(*  Coordfs packs flip into bits 0..11 and slice into 12..23, so the packed   *)
(*  value ranges over 2 ^ 24 while only 2048 x 495 occur.  Ranking the slice  *)
(*  mask instead gives 1 013 760 slots and one array.  srank is 4096 words:   *)
(*  the rank of each twelve bit mask with four bits set, else 495.            *)
(* =========================================================================  *)
...
(* The rank of each twelve bit mask with four bits set, else 495, which is    *)
(* also the default, so a read past the end gives the impossible value        *)
(* rather than the rank of slice 0.  `Eval vm_compute in' is not optional:    *)
(* without it the body is the cons list, and a tactic unfolding a lookup      *)
(* meets a 67 584 cell term.                                                  *)
...
(*                                                                            *)
(*  rubik_par takes the maximum of three lower bounds per view: flip and      *)
(*  slice, twist and slice, and phase 1.  This is the second.  2187 x 495 =   *)
(*  1 082 565 states, all reached by depth 9, so four bits suffice.           *)
(*                                                                            *)
(*  Indexed by mask, not by rank.  The rank does not move on its own, but     *)
(*  the mask does: the slice mask of a moved coordinate depends on the slice  *)
(*  mask alone.  FsTable makes the same trade for flip and slice.  A mask     *)
(*  that cannot occur holds 0, which makes the step condition true there.     *)
(* =========================================================================  *)
...
(* The mask loop is an all_pow over twelve bits and never leaves int63; only  *)
(* the twist loop runs of_nat, once per twist.  The proof is long enough to   *)
(* live in its own file and go through native_cast_no_check.  all_pow 9       *)
(* covers 512 ranks, so the 17 past 495 are guarded out.                      *)
...
(* =========================================================================  *)
(*  4.  The table itself: twenty eight bits an entry, two to a word           *)
(*                                                                            *)
(*  Four of those bits are the distance, holding the true distance capped at  *)
(*  p1cap + 1, never an offset clamped from below: that would overstate the   *)
(*  distance for a state that is nearer, and could prune away a real          *)
(*  solution.  Capping from above is the safe direction, and is what          *)
(*  stopping the search at p1cap does.  Nothing below reads the cap: the two  *)
(*  checks are what make the heuristic admissible, whatever the table holds.  *)
(* =========================================================================  *)

Definition p1cap     := 12.           (* the cap mkfold.sh passes to p1gen    *)
...
(* The table is a parameter, as Fstab takes it and FsTable supplies it.       *)
(* Nothing below assumes anything about it -- length, default or contents --  *)
(* because the two checks are what make the heuristic admissible, whatever    *)
(* the array holds.  The three fold tables are parameters on the same terms,  *)
(* and ride in the same array, in the three slots after the distance chunks.  *)
...
(* The shift is an int63 literal, not of_nat of a nat, which p1get would      *)
(* otherwise walk twice on every read.  cwlogiE checks it is the same number. *)
...
(* An entry is twenty eight bits, two to a word: four of distance and four    *)
(* for each of the six faces, saying which moves bring the state nearer H.    *)
(* The faces are for the search alone; RowMask takes them apart.              *)
...
(*                                                                            *)
(*  The shape Coordfs uses: the table is never asked to be correct, only to   *)
(*  be a local certificate.  The check is a forall over independent entries,  *)
(*  so it splits into generated slices as Fsmain does for flip and slice.     *)
(* =========================================================================  *)
...
(* The total twist.  cubcP says the corners turn rigidly, not that the eight  *)
(* orientations sum to 0 mod 3.  The coordinate keeps seven digits and        *)
(* recovers the eighth from that sum, so it has to be carried: it is a        *)
(* property of the cube group, not of rigidity.                               *)
...
(*                                                                            *)
(*  coordtwM and coordtw_step carry `twsum g = 0'.  It is discharged here as  *)
(*  an invariant: the identity has total twist 0, and no move changes it.     *)
(*                                                                            *)
(*  A move sends the U/D slot of corner p to slot cdelta m p of corner        *)
(*  csrc m p, so summing corientg (g * m) over the eight corners re-sums      *)
(*  corientg g over a permutation of them, shifted by the total of the        *)
(*  deltas.  The permutation leaves the sum alone, and the total delta of     *)
(*  every move is 0 mod 3.  Both are table facts, checked below.              *)
(* =========================================================================  *)
...
(*  6.  THE CHECK                                                             *)
(*                                                                            *)
(*  The table is asked only to be a local certificate: zero at the identity,  *)
(*  and never dropping by more than one across a move.  Those two make the    *)
(*  heuristic admissible, and both are closed booleans the kernel evaluates.  *)
(*  This is Fstab's check0 and checkStep, one level up.                       *)
(*                                                                            *)
(*  The loop is over ranks: only 6 % of the 2 ^ 24 packed values occur, so    *)
(*  packed values would be sixteen times the work.  unranki turns a rank      *)
(*  back into a packed value and p1checkUnrank makes that legitimate.         *)
(*  p1mdata is lifted out of the lambda, or it is rebuilt at every state.     *)
(* =========================================================================  *)

Definition p1mdata : seq (nat * mdatf) :=
  [seq (k, mdatf_of_tab (nth [::] mtabs k)) | k <- iota 0 18].

(* This loop runs over packed values rather than ranks.  Ranking first would  *)
(* be sixteen times fewer iterations, but the checked instance would be at    *)
(* unranki (fsidx x) rather than at x, and closing that gap needs fsidx       *)
(* injective on the summaries, which is not available.  Over packed values    *)
(* all_powP hands back the instance at x and the proof is                     *)
(* Fstab.DfsStep_of_check verbatim.                                           *)
(*                                                                            *)
(* The guard is fsok, not `fsidx x <? nfsi': see where fsok is defined.       *)
(* Without a guard, all 2 ^ 24 packed values would run the eighteen move      *)
(* checks instead of the 6 % that are summaries.                              *)
...
(* These four belong in Coordfs, which owns eprim, esec and epair; they are   *)
(* here so that closing fsidx_lt does not rebuild the certificate files.      *)
(* The cubP hypothesis is needed: for an arbitrary permutation the count is   *)
(* anything at all and fsidx_lt is false.                                     *)
...
(*   - nfs appears as the int63 literal nfsi, never as of_nat nfs, which is   *)
(*     a million reduction steps inside the loop;                             *)
(*   - to_nat nfsi = nfs cannot be proved: it builds the unary nat and        *)
(*     overflows the stack.  The fsidx bound is stated in int63 instead.      *)
(*                                                                            *)
(* For the guard, from fsL : (fsidx x <? nfsi) show (nfsi <=? fsidx x) =      *)
(* false: apply/idP/negP to put both inequalities in the context, nlebP and   *)
(* nltbP to move to nat, then leqNgt.  case: ifP does not return.             *)
...
(*                                                                            *)
(*  It cannot go through Coordfs.hcoordg, which wants Dstep for every x.      *)
(*  Fstab has that, because a coordinate past the table reads 0; here the     *)
(*  rows are contiguous, so an out of range rank reads a good entry for a     *)
(*  different state.  The guard is discharged by hand from the invariant,     *)
(*  which is what fsidx_lt is for.                                            *)
(* =========================================================================  *)
...
(* The rewrite patterns are needed: the two reads are at different types,     *)
(* array (array int) and array int, and a bare rewrite matches neither.       *)

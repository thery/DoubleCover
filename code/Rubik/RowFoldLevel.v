(* =========================================================================  *)
(*  RowFoldLevel.v -- what ONE level costs, before anything is guessed.      *)
(* =========================================================================  *)
...
(* ONE LEVEL, THEN TWO, THEN THREE.  Below the row's own depth a level
   neither searches nor spreads, so what these three time is the sweep and
   the count and nothing else -- and the difference between two of them is
   one level, measured rather than estimated. *)

Time Eval native_compute in frunl 1 0 (mkempty tt) (mkempty tt) [::].

Time Eval native_compute in frunl 2 0 (mkempty tt) (mkempty tt) [::].

Time Eval native_compute in frunl 3 0 (mkempty tt) (mkempty tt) [::].

(* and the first depth that does anything *)

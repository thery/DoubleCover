(* =========================================================================  *)
(*  Fold.v -- the phase 1 table folded by the sixteen symmetries of the       *)
(*            U/D axis, and the certificate it has to carry.                  *)
(*                                                                            *)
(*  The unfolded table has one entry per (twist, flip x slice rank) pair.     *)
(*  A symmetry that fixes the U/D axis acts on both coordinates at once, so   *)
(*  one row per orbit of ranks is enough: a state is read by mapping its      *)
(*  rank to the orbit it belongs to and its twist through the symmetry that   *)
(*  takes the rank to the orbit representative.                               *)
(*                                                                            *)
(*  NOTHING HERE ASKS THE TABLE TO BE CORRECT.  Exactly as in Phase1.v, the   *)
(*  folded read only has to be zero at the identity and to drop by at most    *)
(*  one across a move, and those two facts make the heuristic admissible.     *)
(*  The point of the file is that the second one need only be CHECKED at the  *)
(*  orbit representatives: DfoldiS carries it to every rank.                  *)
(*                                                                            *)
(*  The data is abstract -- tables as section variables, their properties as  *)
(*  hypotheses -- so the file compiles with no table emitted.                 *)
(* =========================================================================  *)

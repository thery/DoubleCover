...
- `Sseq` is the list of the eighteen moves, in the order the search walks over
...
distances, with one condition on the summary and two on the table.
...
since only it says whether the cube is solved. The refined search also plays
fewer moves: turning the same face twice running is never useful, and the next
section gives the other rules of that kind. Here is its shape, with simplified
names and some details left out; the real one is `searchz3` in
#src("Farp1.v"):
...
Two things travel down the tree instead of one, and each has its own job:
...
The table is generated by an OCaml program and gives the exact distance, but the
proof does not need that. It checks the two statements above and nothing else.
Together they make the estimate a lower bound on the moves still needed, which
is what allows a branch to be cut. A table of zeros would pass both: it would
prune nothing and the search would run for ever, but the answer would still be
right.

So the generator is not trusted. It writes the table out as Rocq source, and the
two statements are checked on it afterwards. The entries do not depend on each
other, so the second check is cut into slices, one file each, and the slices are
checked at the same time. It takes ten minutes.

Every timing in this note is measured on the same machine, the _reference
machine_: a dual-socket Intel Xeon E5-2667 at 2.9 GHz, twelve cores,

(* =========================================================================  *)
(*  RowCubInst48.v -- the row again, with the twenty cubies carried.            *)
(* =========================================================================  *)
...
Require Import Row48 RowMap48 RowRun48 RowFinal48 RowSrch48 RowSrchP48.
Require Import RowPrep48 RowMark48.
...
(* what the corner pair asks for: the flip is a parity, and the class table  *)
(* and the flip are the page table split                                     *)
...
Proof. by apply: (RowPrep48.hcfl hcflok). Qed.
...
Definition ymfin : PArray.array arr :=
  run e8num e4bit par8 cpg cfl mgr msw mlo mhi p1 (RowInst.cstep fsstep) zstepi
      ytomemb okmv ycsolved RowInst.croot yrooti dsrch nlev 0
      (mkempty48 tt) (mkempty48 tt).

Lemma ymfin_sound :
  soundat e8inv e4of par4
...
apply: (run_sound he8 he4 ycoord_root yroot_ball yroot_pok ycoord_step
                  ...
                  RowInst.hmv_Sset (grpmvP48 (RowInst.grpmvP hsrc hhalf) hcfl)
                  (prep_move48 he8 hcflok hcpg (RowInst.prep_move memb2tab_ok hpg hgr hbt memb2tab_move)));
  exact: sound_mempty48.
Qed.

(* ---- and the row --------------------------------------------------------- *)

Theorem yrow_within_20 : nlev = 20%N ->
  witsok48 e8inv e4of par4 (RowInst.ptab memb2tab) wl ->
  mfull248 ymfin (wmap48 wl) ->
  forall x, membok par8 par4 x ->
  RowRun.wthn (RowFinal.pos (RowInst.ptab memb2tab)) 20 x.
Proof.
move=> hn hw hf x hx.
apply: (row48_within_20 he8 he4 (RowInst.ptab_ok memb2tab_ok) _ hw hf hx).
...
(* wmap is a map the size of this one holding thirty two bits, and mfull248     *)

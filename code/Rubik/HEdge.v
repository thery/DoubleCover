...
(* Both halves of the step are now in hand: getn_step says which sticker is   *)
(* read after the turn, getn_sec says the cubie is the same whichever of the  *)
(* two facelets of its place it is read from.  What is left is to put them    *)
(* together -- eplace after the turn at place j is eplace before it at place  *)
(* eplc m j, and the flip picks up eflp m j -- then eslot, then the datum,    *)
(* and then the sweep over the 190080 data ties the rule to the table.        *)

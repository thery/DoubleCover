(* =========================================================================  *)
(*  RowSrchP48.v -- the plain search with the cuts and the stop is sound.       *)
(* =========================================================================  *)
...
Require Import Row RowMap RowRun Fold RowMask RowSrch48.
Require Import Row48 RowMap48 RowRun48 RowSrch48.

(* THIS IS THE SAME FILE OVER Row48's LAYOUT.  A file of its own because the  *)
(* original names place and unplace directly, and editing it would move its   *)
(* .vo and the runs banked on it.  What changed: the layout, the width of a   *)
(* cell, and that a page is a corner PAIR.                                    *)
...
Hypothesis prep_eq : forall m dst,
  prep m dst = prepass48 cpg cfl mgr msw mlo mhi m dst.
...
Local Notation srchk := (RowSrch48.srchk e8num e4bit par8 F frep fsym twsym
                           dnlo dnhi fllo flhi cstep xstep tomemb okmv
                           csolved ishm).
Local Notation srchsk := (RowSrch48.srchsk e8num e4bit par8 F frep fsym twsym
                            dnlo dnhi fllo flhi cstep xstep tomemb okmv
                            csolved ishm).
Local Notation levelsk := (RowSrch48.levelsk e8num e4bit par8 prep
                             F frep fsym twsym dnlo dnhi fllo flhi
                             cstep xstep tomemb okmv csolved croot sroot
                             dsrch ishm).
Local Notation runsk := (RowSrch48.runsk e8num e4bit par8 prep
                           F frep fsym twsym dnlo dnhi fllo flhi
                           cstep xstep tomemb okmv csolved croot sroot
                           dsrch ishm).
...
Hypothesis grpmvP : forall k v bt', (to_nat k < nhn)%N ->
  (bt' <? nbit48i) ->
  ~~ (Uint63.land (RowMap48.grpmv48 cfl msw mlo mhi k v) (bitof bt') =? 0) ->
  exists2 bt, (bt <? nbit48i) &
    btmv k bt = bt' /\ ~~ (Uint63.land v (bitof bt) =? 0).

Hypothesis prep_move : forall k pg gr bt, (to_nat k < nhn)%N ->
  inrange48 pg gr bt ->
  inrange48 (RowMap48.cpgmv cpg k pg) (RowMap.grmv mgr k gr) (btmv k bt) /\
  pos (unplc (RowMap48.cpgmv cpg k pg) (RowMap.grmv mgr k gr) (btmv k bt))
  = (pos (unplc pg gr bt) * hmv k)%g.

(* ---- the search with the cuts, which is RowRun48.srch_sound with two more    *)
...
elim: togo c x msk pv m => [|togo ih] c x msk pv m hdt hc hp hm hb.
  ...
  case: (mmark48P (place48_range he8 he4 hok E) hr ht) => [[<- <- <-]|hb2];
    last by apply: hm.
  rewrite /RowRun.wthn (unplace48_place48 he8 he4 hok E) (leaf_pos hc hp hG hs).
...
Lemma srchsk0 cut c x msk pv enough mn :
  ...
  (if Uint63.leb enough mn.2 then mn
   ...
   then mmarkn mn (Uint63.div (PArray.get e8num (mcp (tomemb x))) 2)
                  (Uint63.div (PArray.get e8num (mud (tomemb x))) 2)
                  (Uint63.add (Uint63.mul (PArray.get par8 (mcp (tomemb x)))
                                          nbiti)
...
have hp := RowRun48.prepass_sound hmv_Sset grpmvP prep_move hm hd.
...
apply: ih.
  by apply: levelsk_sound => //; apply: RowRun48.soundatW.
by apply: RowRun48.soundatW.
Qed.
...
Local Notation srchki := (RowSrch48.srchki e8num e4bit par8 F frep fsym twsym
                            dnlo dnhi fllo flhi cstep xstep tomemb okmv
                            csolved ishm).
Local Notation srchski := (RowSrch48.srchski e8num e4bit par8 F frep fsym twsym
                             dnlo dnhi fllo flhi cstep xstep tomemb okmv
                             csolved ishm).
Local Notation levelski := (RowSrch48.levelski e8num e4bit par8 prep
                              F frep fsym twsym dnlo dnhi fllo flhi
                              cstep xstep tomemb okmv csolved croot sroot
                              dsrch ishm).
Local Notation runski := (RowSrch48.runski e8num e4bit par8 prep
                            F frep fsym twsym dnlo dnhi fllo flhi
                            cstep xstep tomemb okmv csolved croot sroot
                            dsrch ishm).
...
rewrite /RowSrch48.srchki -/RowSrch48.srchki /RowSrch48.srchk -/RowSrch48.srchk.
...
rewrite /RowSrch48.srchski -/RowSrch48.srchski /RowSrch48.srchsk -/RowSrch48.srchsk.
...
Proof.
move=> hb; rewrite /RowSrch48.levelski -/RowSrch48.levelski.
rewrite /RowSrch48.levelsk -/RowSrch48.levelsk; cbv zeta.
case: ifP => _; last by [].
have hdw : (d < nwB)%N.

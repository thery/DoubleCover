...
(* Carrying twenty numbers rather than the forty eight entry facelet table    *)
(* takes most of the position's cost out of the search.                       *)
(*                                                                            *)
(* The twenty have to name a cube, which means orientation: a corner sits at  *)
(* a place turned by nought, one or two, and an edge flipped or not.  The     *)
(* builder is RowMemb's `part' with the slot moved on by the turn, which is   *)
(* `partt' below.  The edges needed a layout of their own, elay, holding the  *)
(* twelve in one: RowMemb's split into outer eight and middle four means      *)
(* something only inside H.                                                   *)
(*                                                                            *)
(* What is proved: a move on the twenty is the move on the cube they name,    *)
(* for every twenty that name places.  What is checked and not proved: that   *)
(* reading a table into twenty numbers and building it back gives the table.  *)
...
(* A move's own twenty are the step table: at place p, which place the cubie  *)
(* comes from and how far it is turned on the way.  Stepping is one lookup    *)
(* and one addition round the place, twenty of each.  It is the inverse that  *)
(* is wanted: composing the move on the right, which is what RowRun asks,     *)
(* means looking the cubie up through the move undone.                        *)

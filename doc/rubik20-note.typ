...
cubes, and summaries are few enough to compute the exact distance of each one to
...
needed, which is the condition for cutting a branch. A table of zeros would pass both.
It would prune nothing and the search would run for ever, but the answer would
still be right. So the generator is not trusted. It writes the table out as Rocq
source, and the two statements are checked on it afterwards. The entries do not
depend on each other. So the second check is cut into slices, one file each, and
the slices are checked at the same time. It takes ten minutes. Every timing in
this note is measured on the same machine, the *reference machine*: a
...
These two neighbours, one turn from $cal(A)$ and one from $cal(C)$, may sit
anywhere in the word, while a prefix has to be at the front. The cyclic shift
moves them there. Let $r$ be the relabelling that
...
The two words have the same length. This holds because $r$ leaves superflip4
unchanged, and the position was chosen for that reason. The shift also
preserves the two sets. $r$ fixes $U$ and $D$ and permutes the other four
faces, so a turn of $cal(A)$ stays in $cal(A)$ and a turn of $cal(C)$ stays in
$cal(C)$. The two turns now at the front are still one from each set.
